...
void ProtocProcess::TryParse(const ShareData& data, std::vector<ShareFrame>& frameList)
{
	for (unsigned int i = 0; i < data->Used();)
	{
		...
		case ProtocProcess::WAIT_CMD_DATA: {
			...
			if (m_Frame.m_u32length > (unsigned)m_DataLen)
			{
				m_Status = WAIT_CMD_DATA;
			}
			else
...
void ProtocProcess::TryPack(const ShareFrame& frame, ShareData& data)
{
	...
	data->Data()[0] = frame->m_length[3];
	data->Data()[1] = frame->m_length[2];
	data->Data()[2] = frame->m_length[1];
	data->Data()[3] = frame->m_length[0];
	data->Data()[4] = frame->m_version;
	data->Data()[5] = frame->m_sequence[3];
	data->Data()[6] = frame->m_sequence[2];
	data->Data()[7] = frame->m_sequence[1];
	data->Data()[8] = frame->m_sequence[0];
	data->Data()[9] = frame->m_cmdType;
	data->Data()[10] = frame->m_cmdNum;
